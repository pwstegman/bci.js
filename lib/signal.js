 ...
 * The data array is passed to the callback function.
 * @param {string} [dimension] - Can be either "channels" or "samples". A value of "channels" will
 * store the added data such that values of the returned 2d array are indexed as [channel_number][sample_number].
 * Passing a dimension of "samples" will instead index values in the array as [sample_number][channel_number].
 */
function EEGWindow(size, numChannels, callback, dimension) {
	this.size = size;
	this.length = 0;
	this.callback = callback;
	this.store = [];
	this.dimension = typeof dimension !== 'undefined' ? dimension : "channels";
