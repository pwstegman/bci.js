...
function bandpower(samples, sample_rate, bands, options) {
    let {fftSize, average, input, relative} = Object.assign({
    ...
    if(relative) {
        total_power = integrate(psd.estimates, {rule: '3/8', dx: dx});
    }
    ...
    for(let i = 0; i < bands.length; i++) {
        ...
        areas[i] = integrate(psd_band, {rule: '3/8', dx: dx}) / total_power;
    }
