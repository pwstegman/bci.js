...
var _periodogram = _interopRequireDefault(require("./periodogram"));

var _windowApply = _interopRequireDefault(require("../data/windowApply"));

var _nextpow = _interopRequireDefault(require("./nextpow2"));
...
function welch(signal, sample_rate, options) {
  ...
  if (fftSize === null) fftSize = Math.pow(2, (0, _nextpow.default)(segmentLength));
  var step = segmentLength - overlapLength;
  if (step <= 0) throw new Error('Invalid overlap, must be less than segmentLength');
  var PSDs = (0, _windowApply.default)(signal, function (epoch) {
    return (0, _periodogram.default)(epoch, sample_rate, {
      fftSize: fftSize,
      window: window
    });
